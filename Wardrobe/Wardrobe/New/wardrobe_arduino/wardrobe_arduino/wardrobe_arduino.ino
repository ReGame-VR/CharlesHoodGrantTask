...
//LedController ledController(3, LED_PIN);
...
void loop() {
  ...
  if (Serial.available() >= 0)
  {
    ...
    /* Check for start byte to keep in sync */
    if (rx_buffer[0] == 0x55 && bytes_read > 9 )
    {
      for (idx = 0; idx < NUMBER_OF_TARGETS; ++idx)
      {
        command_state.led_colors[idx] = (LedColor_t)rx_buffer[idx+1];
      }
